...
memarg="-server -Xms6g -Xmx6g -Xss256K"
...
ssh ${node} kill $pid
    ...
    echo "afterkill"
    ssh ${node} ps aux|grep $main|awk '{print$2}'
    ssh ${node} nohup /usr/java/jdk/bin/java $fileencoding $memarg $gcarg $verboses -classpath ${runJar}/${jar} $main recovery > /dev/null 2>&1 &
